...
export class ChangeParser {
  ...
  get parsedChange(): string {
    ...
    usd.USD.forEach((d, index) => {
      if (this.result[index].value === 0) {
        ...
        summary += `${this.result[index].value} ${this.parsePenny(
  ...
  provideChange(): void {
    if ((this.owed * 100) % 3 === 0) {
      ...
      let shuffled = usd.USD.map((value) => ({ value, sort: Math.random() }))
  ...
  makeChange(array: Change[]): void {
    let changeInCents: number =
      Number(Number(this.difference).toFixed(2)) * 100;
    array.forEach((el) => {
